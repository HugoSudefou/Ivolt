...
export class ApiService {
  ...
  private _checkReturn<R>(returnApi: AxiosResponse<R> | AxiosError<R>, message?:{ successMessage: string | null, warningMessage: string | null }): WorkDone<R> {
    ...
    if (returnApi && returnApi.status === 200) {
      ...
      return WorkDone.buildOk(response.data, message?.successMessage ?? '', message?.warningMessage ?? '')
    }
  ...
  private static _catchHandler<R>(wd: WorkDone<R>): WorkDone<R> | Promise<WorkDone<R>> {
  ...
  private static _thenHandler<R>(wd: WorkDone<R>): WorkDone<R> | Promise<WorkDone<R>> {
  ...
  public async doGet<R>(serviceUri: string, params?: any, config?: AxiosRequestConfig, message?: { successMessage: string | null, warningMessage: string | null }): Promise<WorkDone<R>> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public async doPost<R>(serviceUri: string, data?: any, config?: AxiosRequestConfig, message?: { successMessage: string | null, warningMessage: string | null }): Promise<WorkDone<R>> {
    const runConfig = _merge({}, config)
    try {
      const resp = await this._axiosInstance.post<R>(serviceUri, data, runConfig)
      return ApiService._thenHandler(this._checkReturn(resp, message))
    } catch (err) {
      return ApiService._catchHandler(this._checkReturn(err as AxiosError<R>))
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public async doDelete<R>(serviceUri: string, data?: any, config?: AxiosRequestConfig): Promise<WorkDone<R>> {
    const runConfig = _merge({}, config)
    try {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  public async doPut<R>(serviceUri: string, data?: any, config?: AxiosRequestConfig): Promise<WorkDone<R>> {
    const runConfig = _merge({}, config)
    try {
      const resp = await this._axiosInstance.put<R>(serviceUri, data, runConfig)
      return ApiService._thenHandler(this._checkReturn(resp))
    } catch (err) {
      return ApiService._catchHandler(this._checkReturn(err as AxiosError<R>))
    }
