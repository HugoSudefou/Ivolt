...
export class StockService extends AbstractFirebaseService<IStockDtos | ITransactionStockDtos> {
   ...
   */
  public async addTransactionStock(transactionStock: ITransactionStockDtos): Promise<WorkDone<IStockDtos | null>> {
    try {
      const idCreateStrock = await this.add(transactionStock);
      const stock = await this.get(idCreateStrock);
      if (stock) {
        const newStock = await this.mergeReferencedData(stock as IStockDtos, 'user') as IStockDtos;
        newStock.id = idCreateStrock;
        return this._thenHandler(WorkDone.buildOk(newStock, MessageEnum.IS_OK_ADD_TRANSACTION_STOCK));
      }
      return this._catchHandler(WorkDone.buildError(MessageEnum.IS_NOK_ADD_TRANSACTION_STOCK));
    } catch (e) {
