...
export default defineComponent({
  ...
  setup() {
    ...
    const optionAction = ref<ITypeActionStockEnum[] | null>([ITypeActionStockEnum.ENTREE, ITypeActionStockEnum.SORTIE])
    ...
    function getUserForTransacStock() {
      ...
      if (users.value) {
        if (userStore.rang.label === 'Directeur') {
          ...
          const userStock = convertUserToIUserDto(users.value.find(user => user.firstName === 'STOCK ENTREPRISE'))
    ...
    async function onSubmit() {
      ...
      if (isErrorAction.value || isErrorNbBatterie.value || isErrorUser.value) return;
      spinnerStore.incrementPendingRequests();
      const transactionStock = convertIFormTransactionStockDtosToITransactionStockDtos(formTransactionStock);
      const wdAddTransacStock = await stockService.addTransactionStock(transactionStock)
    ...
    const isErrorUser = computed(() => {
      return formTransactionStock.user === null || formTransactionStock.user.id === '0'
    });
    ...
    return {
      ...
      filterFn(val: any, update: any) {
        ...
        update(() => {
          ...
          if (users.value) {
            options.value = users.value.filter(v => v.firstName.toLowerCase().indexOf(needle) > -1 || v.lastName.toLowerCase().indexOf(needle) > -1)
          }
        })
      },
