...
export class WorkDone<T> {
  ...
  public static async buildOkIf<T>(
    valueToTest: any,
    errorMsg: string,
    fn: (data: any) => Promise<T>,
  ): Promise<WorkDone<T>> {
    return _isNil(valueToTest)
      ? WorkDone.buildError(errorMsg)
      : WorkDone.buildOk(await fn(valueToTest));
  }
  ...
  public static okIf(valueToTest: any, errorMsg: string): WorkDone<boolean> {
    return _isNil(valueToTest) || valueToTest === false
      ? WorkDone.buildError(errorMsg)
      : WorkDone.buildOk(true);
  }

  public static okWithDataIf<T>(valueToTest: T, errorMsg: string): WorkDone<T> {
    return _isNil(valueToTest)
      ? WorkDone.buildError(errorMsg)
      : WorkDone.buildOk(valueToTest);
  }
}
